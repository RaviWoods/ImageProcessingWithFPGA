...
{
    ac_int<16, false> red, green, blue, grid1[KERNEL_WIDTH], grid2[KERNEL_WIDTH], gout[KERNEL_WIDTH];
    double process = 0;
    ...
    FRAME: for(int p = 0; p < NUM_PIXELS; p++) {
		...
		ACC_GY: 
			...
			for(i = 0; i < KERNEL_WIDTH; i++) {
				...
				if (i == 1) {
					grid1[0] += (0)*(regs[1].slc<PIXEL_WL>(PIXEL_WL));
					grid1[1] += (0)*(regs[1].slc<PIXEL_WL>(PIXEL_WL));
					grid1[2] += (0)*(regs[1].slc<PIXEL_WL>(PIXEL_WL));
				}
				if (i == 2) {
					grid1[0] += (1)*(regs[2].slc<PIXEL_WL>(2*PIXEL_WL));
					grid1[1] += (2)*(regs[2].slc<PIXEL_WL>(2*PIXEL_WL));
					grid1[2] += (1)*(regs[2].slc<PIXEL_WL>(2*PIXEL_WL));
				}
			}
		...
		ACC_GX: 
			for(i = 0; i < KERNEL_WIDTH; i++) {
				if(i == 0) {
					grid2[0] += (-1)*(regs[0].slc<PIXEL_WL>(0));
					grid2[1] += (0)*(regs[0].slc<PIXEL_WL>(0));
					grid2[2] += (1)*(regs[0].slc<PIXEL_WL>(0));
				}
				if (i == 1) {
					grid2[0] += (-2)*(regs[1].slc<PIXEL_WL>(PIXEL_WL));
					grid2[1] += (0)*(regs[1].slc<PIXEL_WL>(PIXEL_WL));
					grid2[2] += (2)*(regs[1].slc<PIXEL_WL>(PIXEL_WL));
				}
				if (i == 2) {
					grid2[0] += (-1)*(regs[2].slc<PIXEL_WL>(2*PIXEL_WL));
					grid2[1] += (0)*(regs[2].slc<PIXEL_WL>(2*PIXEL_WL));
					grid2[2] += (1)*(regs[2].slc<PIXEL_WL>(2*PIXEL_WL));
				}
			}
