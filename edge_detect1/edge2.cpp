...
// #if 0: copy input to output bypassing filter
    ...
    FRAME: for(int p = 0; p < NUM_PIXELS; p++) {
		...
		ACC_GX: 
			...
			grid2[3] += (-2)*(regs[1].slc<PIXEL_WL>(PIXEL_WL));
			grid2[4] += (0)*(regs[1].slc<PIXEL_WL>(PIXEL_WL));
			grid2[5] += (2)*(regs[1].slc<PIXEL_WL>(PIXEL_WL));
			grid2[6] += (-1)*(regs[2].slc<PIXEL_WL>(2*PIXEL_WL));
			grid2[7] += (0)*(regs[2].slc<PIXEL_WL>(2*PIXEL_WL));
			grid2[8] += (1)*(regs[2].slc<PIXEL_WL>(2*PIXEL_WL));

		
		
		// add the accumualted value for all processed lines
		ACC2: for(i = 0; i < KERNEL_WIDTH; i++) {  
			gout[i] = sqrt((grid1[i]*grid[i]) + (grid2[i]*grid2[i]));
			red += gout[i];
			green += gout[i];
			blue += gout[i];
		}
