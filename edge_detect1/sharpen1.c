...
// #if 0: copy input to output bypassing filter
	...
	const ac_int<1,true> gx[9] = {0,-1,0,-1,5,-1,0,-1,0};
    ...
    FRAME:
		...
		ACC_GX: for(i = 0; i < KERNEL_WIDTH; i++) {
					for(a = 0; a < KERNEL_WIDTH; a++) {
						redarray[a] += gx[(a+(3*i))]*(regs[i].slc<COLOUR_WL>(2*COLOUR_WL + a*PIXEL_WL));
						greenarray[a] += gx[((3*i)+a)]*(regs[i].slc<COLOUR_WL>(COLOUR_WL + a*PIXEL_WL));
						bluearray[a] += gx[((3*i)+a)]*(regs[i].slc<COLOUR_WL>(0  + a*PIXEL_WL));
					}
				}
